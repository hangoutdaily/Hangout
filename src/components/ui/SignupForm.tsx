...
export default function SignupForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setUser(res.data.authUser || res.data.user);
  ...
  const handleGoogleLogin = useGoogleLogin({
    ...
    onSuccess: async (codeResponse) => {
      ...
      try {
        ...
        setErrors({
          email: error.response?.data?.error || 'Google signup failed. Please try again.',
        });
      } finally {
    ...
    onError: () => {
      setErrors({ email: 'Google signup was cancelled or failed.' });
    },
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <button
        ...
        className="w-full border border-border rounded-lg py-3 font-medium hover:bg-surface transition-colors flex items-center justify-center gap-2 cursor-pointer disabled:opacity-70"
      >
        {isGoogleLoading ? (
