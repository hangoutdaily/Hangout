...
export default function CategoryFilters() {
  ...
  const nextFourDates = useMemo(() => {
    ...
    const isoFmt = new Intl.DateTimeFormat("en-CA", { year: "numeric", month: "2-digit", day: "2-digit" });
  ...
  return (
    <div className="mx-auto max-w-7xl" ref={wrapperRef}>
      {/* Filters */}
      <div className="relative flex items-center justify-center mb-4 gap-2 pb-2">
        {/* Type */}
        <button
          onClick={() => {
            setIsTypeOpen(v => !v);
          ...
          className="flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium whitespace-nowrap bg-surface text-foreground hover:bg-accent/10"
        >
          ...
          {selectedTypes.length > 0 && (
            <span className="ml-1 rounded-full bg-accent text-accent-foreground px-2 py-0.5 text-xs">
        ...
        {/* TYPE MODAL (centered below all filters) */}
        {isTypeOpen && (
